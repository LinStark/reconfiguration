...
// send a msg into the receiveRoutipackage consensus
//
//import (
//	"bytes"
//	"fmt"
...
//		// Increment Proposer Priority 增加proposer优先级
//	}
//
//	// Setup new round
...
//		logger.Debug("This node is done")
...
//	//如果我们足够快没有获得提案或者全部区块部分，那就进入enterprevote
...
//	//制作一个区块
//	//commit是一组valiators的集合
//	var commit *types.Commit
//	if cs.Height == 1 {
//		// 如果是第一个区块，那么commit就置为空
//
...
//	}
//	return 0
//}ne regarding our own proposal, block part, or vote
// 发送信息到读协程可能提案、部分区块、投票信息
func (cs *ConsensusState) sendInternalMessage(mi msgInfo) {
	select {
	case cs.internalMsgQueue <- mi:
...
func (cs *ConsensusState) addVote(vote *types.Vote, peerID p2p.ID) (added bool, err error) {
	//将收到票据进行添加
	//输出一些基本信息
	cs.Logger.Debug("addVote", "voteHeight", vote.Height, "voteType", vote.Type, "valIndex", vote.ValidatorIndex, "csHeight", cs.Height,"voteround",vote.Round)

	// A precommit for the previous height?
	// These come in while we wait timeoutCommit
	//这种情况说明收到了一个过去的投票。比如现在区块高度是2，收到一个vote的高度为1，说明这一轮次大家都达成了共识，没必要对这个投票进行处理。
	if vote.Height+1 == cs.Height {
		if !(cs.Step == cstypes.RoundStepNewHeight && vote.Type == types.PrecommitType) {
			// TODO: give the reason ..
			// fmt.Errorf("tryAddVote: Wrong height, not a LastCommit straggler commit.")
